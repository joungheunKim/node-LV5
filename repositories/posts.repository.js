...
class PostRepository {
    ...
    findOne = async (target) =>{
        return await Posts.findOne({where: {[Op.and]: target}, include: [{ model: Users }], attributes: { include: [[Sequelize.literal(`(SELECT COUNT(*) FROM Likes WHERE PostId = Post.postId)`), 'like']] }, raw: true, nest: true } )
    }
